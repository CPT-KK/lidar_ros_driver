...
void calculateDimPos(const pcl::PointCloud<pcl::PointXYZI>& cluster, geometry_msgs::Pose& output, double& theta_star) {
    ...
    std::cout << "angle: " << std::atan2(e_1_star.y(), e_1_star.x()) * 180.0 / 3.14 << std::endl;
    std::cout << "length: " << std::fabs(e_x.dot(diagonal_vec)) << std::endl;
    std::cout << "width: " << std::fabs(e_y.dot(diagonal_vec)) << std::endl;
...
#pragma omp parallel for num_threads(8)
    ...
    for (const auto& cluster : cluster_indices) {
        ...
        // remove the outlier
        orientation_calc orient_calc_("VARIANCE");  // AREA
        double theta_optim;
